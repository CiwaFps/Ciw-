...
CreateToggle("ESP", 0, function(state)
_G.Master = state
end)
...
CreateRow("Nama + Jarak", "ShowName", Color3.fromRGB(255, 50, 50), 1)
...
local function UpdateDistance(label, targetPlayer)
    task.spawn(function()
        while label and label.Parent do
            if _G.Master and _G.ShowName and targetPlayer.Character and targetPlayer.Character:FindFirstChild("HumanoidRootPart") and LP.Character and LP.Character:FindFirstChild("HumanoidRootPart") then
                local dist = (LP.Character.HumanoidRootPart.Position - targetPlayer.Character.HumanoidRootPart.Position).Magnitude
                label.Text = string.format("%s\n[%.1f m]", targetPlayer.DisplayName or targetPlayer.Name, dist)
            else
...
local function LabelESP(p)
    ...
    local function apply(char)
        ...
        -- tag.Enabled jangan diisi di sini, biar loop di bawah yang atur
        ...
        task.spawn(function()
            while tag and tag.Parent do
                tag.Enabled = (_G.Master == true and _G.ShowName == true)
                task.wait(1)
            end
        end)
        UpdateDistance(tl, p)
...
for _, p in pairs(Players:GetPlayers()) do LabelESP(p) end
Players.PlayerAdded:Connect(LabelESP)
